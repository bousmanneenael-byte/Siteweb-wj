...
function newPrices(){
 Object.keys(countryData).forEach(id=>{
   ...
   if(leaderType[id]) prices[id][leaderType[id]] = r(1000,10000);
 });
}
...
function saveGame() {
    ...
    localStorage.setItem("lordSave", JSON.stringify(save));
}
function loadGame() {
    let save = localStorage.getItem("lordSave");
    if(save){
...
function update(){
 ...
 saveGame(); // Sauvegarde après update
}
